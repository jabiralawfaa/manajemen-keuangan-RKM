...
```bash
curl -X DELETE http://localhost:5000/api/delete/members/1 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```

### Menghapus Pembayaran (oleh Ketua)
```bash
curl -X DELETE http://localhost:5000/api/delete/payments/1 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```

### Menghapus Pengeluaran (oleh Ketua)
```bash
curl -X DELETE http://localhost:5000/api/delete/expenses/1 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
