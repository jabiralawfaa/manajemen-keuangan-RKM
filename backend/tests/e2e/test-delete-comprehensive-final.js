// tests/e2e/test-delete-comprehensive-final.js
...
async function testDeleteComprehensiveFinal() {
  ...
  try {
    ...
    console.log('\n📋 Menguji DELETE /api/members/:id (oleh ketua)');
    try {
      if (memberId) {
    ...
    try {
      ...
      try {
        const response2 = await axios.delete(`${BASE_URL}/api/members/${testMemberId}`, {
          headers: { 'Authorization': `Bearer ${tokens.sekretaris}` }
        });
        ...
        if (deleteError.response && deleteError.response.status === 403) {
          console.log('   ✅ DELETE /api/members/:id (oleh sekretaris): Ditolak dengan benar - Status:', deleteError.response.status);
          console.log('      Pesan:', deleteError.response.data.message);
        } else {
          console.log('   ❌ DELETE /api/members/:id (oleh sekretaris): Error tak terduga -', deleteError.response?.data?.message || deleteError.message);
        }
      }
    } catch (createError) {
    ...
    // Test DELETE /api/payments/:id (oleh bendahara - seharusnya berhasil)
    console.log('\n💳 Menguji DELETE /api/payments/:id (oleh bendahara)');
    try {
      if (paymentId) {
        const response = await axios.delete(`${BASE_URL}/api/payments/${paymentId}`, {
          headers: { 'Authorization': `Bearer ${tokens.bendahara}` }
        });
        console.log('✅ DELETE /api/payments/:id (oleh bendahara): Berhasil - Status:', response.status);
        console.log('   Pesan:', response.data.message);
      } else {
        console.log('⚠️  Tidak ada pembayaran untuk dihapus');
      }
    } catch (error) {
      console.log('❌ DELETE /api/payments/:id (oleh bendahara): Gagal -', error.response?.data?.message || error.message);
    }

    // Test DELETE /api/payments/:id (oleh sekretaris - seharusnya gagal)
    console.log('\n   2. Menguji DELETE /api/payments/:id (oleh sekretaris - seharusnya gagal)');
    try {
      ...
      if (membersResponse2.data.members && membersResponse2.data.members.length > 0) {
        ...
        try {
          const response2 = await axios.delete(`${BASE_URL}/api/payments/${testPaymentId}`, {
            headers: { 'Authorization': `Bearer ${tokens.sekretaris}` }
          });
          console.log('   ❌ DELETE /api/payments/:id (oleh sekretaris): Seharusnya gagal tapi berhasil - Status:', response2.status);
        } catch (deleteError) {
          if (deleteError.response && deleteError.response.status === 403) {
            console.log('   ✅ DELETE /api/payments/:id (oleh sekretaris): Ditolak dengan benar - Status:', deleteError.response.status);
            console.log('      Pesan:', deleteError.response.data.message);
          } else {
            console.log('   ❌ DELETE /api/payments/:id (oleh sekretaris): Error tak terduga -', deleteError.response?.data?.message || deleteError.message);
          }
        }
      } else {
    ...
    // Test DELETE /api/expenses/:id (oleh bendahara - seharusnya berhasil)
    console.log('\n💰 Menguji DELETE /api/expenses/:id (oleh bendahara)');
    try {
      if (expenseId) {
        const response = await axios.delete(`${BASE_URL}/api/expenses/${expenseId}`, {
          headers: { 'Authorization': `Bearer ${tokens.bendahara}` }
        });
        console.log('✅ DELETE /api/expenses/:id (oleh bendahara): Berhasil - Status:', response.status);
        console.log('   Pesan:', response.data.message);
      } else {
        console.log('⚠️  Tidak ada pengeluaran untuk dihapus');
      }
    } catch (error) {
      console.log('❌ DELETE /api/expenses/:id (oleh bendahara): Gagal -', error.response?.data?.message || error.message);
    }

    // Test DELETE /api/expenses/:id (oleh sekretaris - seharusnya gagal)
    console.log('\n   2. Menguji DELETE /api/expenses/:id (oleh sekretaris - seharusnya gagal)');
    try {
      ...
      try {
        const response2 = await axios.delete(`${BASE_URL}/api/expenses/${testExpenseId}`, {
          headers: { 'Authorization': `Bearer ${tokens.sekretaris}` }
        });
        console.log('   ❌ DELETE /api/expenses/:id (oleh sekretaris): Seharusnya gagal tapi berhasil - Status:', response2.status);
      } catch (deleteError) {
        if (deleteError.response && deleteError.response.status === 403) {
          console.log('   ✅ DELETE /api/expenses/:id (oleh sekretaris): Ditolak dengan benar - Status:', deleteError.response.status);
          console.log('      Pesan:', deleteError.response.data.message);
        } else {
          console.log('   ❌ DELETE /api/expenses/:id (oleh sekretaris): Error tak terduga -', deleteError.response?.data?.message || deleteError.message);
        }
      }
    } catch (error) {
...
module.exports = { testDeleteComprehensiveFinal };