...
async function testPutEndpoints() {
  console.log('🧪 Memulai pengujian endpoint PUT (Update)...');
  ...
  try {
    ...
    console.log('\n🎉 Pengujian endpoint PUT selesai!');
  } catch (error) {
  ...
  try {
    if (paymentId) {
      const updatedPayment = {
        memberId: paymentsResponse.data.payments[0].member_id,
        paymentDate: '2025-06-15',
        month: '2025-06',
        amount: 55000,
        receiptNumber: paymentsResponse.data.payments[0].receipt_number
      };
...
async function testChangePasswordEndpoints(tokens) {
  ...
  console.log('\n   1. Menguji PUT /api/change-password/change-password (oleh bendahara)');
  try {
    ...
    console.log('   ✅ PUT /api/change-password/change-password (oleh bendahara): Berhasil - Status:', response.status);
    console.log('      Pesan:', response.data.message);
  } catch (error) {
  ...
  try {
    // Ambil ID user bendahara2 untuk reset password
    ...
    // Ambil user lain untuk reset password
    const allUsersResponse = await axios.get(`${BASE_URL}/api/members`, { 
      headers: { 
        'Authorization': `Bearer ${tokens.admin}`,
        'Content-Type': 'application/json'
      } 
    ...
    // Untuk pengujian, kita reset password user bendahara2 (asumsikan id=3)
    const resetPasswordData = {
      newPassword: 'ResetPassword123'
    };

    const response2 = await axios.put(`${BASE_URL}/api/change-password/reset-password/3`, resetPasswordData, { 
      headers: { 
        'Authorization': `Bearer ${tokens.admin}`,
        'Content-Type': 'application/json'
      } 
...
module.exports = { testPutEndpoints };