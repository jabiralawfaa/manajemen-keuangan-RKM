...
const ExpenseForm = ({ isEdit = false }) => {
  ...
  const fetchExpense = async () => {
    try {
      ...
      setFormData({
        ...
        category: expense.category || 'lain_lain',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Gagal menyimpan data pengeluaran: ' + (err.response?.data?.message || err.message));
  ...
  const validateForm = () => {
    ...
    if (!formData.amount) {
      setError('Jumlah pengeluaran harus diisi');
      return false;
    }
    
    if (isNaN(parseFloat(formData.amount)) || parseFloat(formData.amount) <= 0) {
      setError('Jumlah pengeluaran harus berupa angka positif');
      return false;
    }
  ...
  const categories = [
    { value: 'kain_kafan', label: 'Kain Kafan' },
    { value: 'memandikan', label: 'Memandikan' },
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm text-gray-500">
                {isEdit 
                  ? 'Perbarui informasi pengeluaran RKM' 
                  : 'Tambahkan pengeluaran baru ke sistem RKM'}
              </p>
            </div>
