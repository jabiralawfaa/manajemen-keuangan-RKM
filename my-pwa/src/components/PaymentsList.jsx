    ...
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start sm:space-y-4 sm:space-x-4">
          <div>
            <h3 className="text-lg leading-6 font-medium text-gray-900">Daftar Pembayaran (Pemasukan)</h3>
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              Kelola data pembayaran iuran anggota Rukun Kematian Muslim
            </p>
          </div>
            ...
            {(user?.role === 'ketua' || user?.role === 'bendahara') && (
              <button
                onClick={() => {
                  ...
                  window.location.href = `${import.meta.env.VITE_REACT_APP_API_URL || 'http://localhost:5000'}/api/export/payments?month=${selectedMonth}`;
                }}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                      ...
                      {(user?.role === 'ketua' || user?.role === 'bendahara') && (
                        <>
                          <button
                            onClick={() => window.location.href = `/payments/edit/${payment.id}`}
            ...
            <div>
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">
                  {Math.min(currentPage * itemsPerPage, payments.length + (currentPage - 1) * itemsPerPage)}
                </span>{' '}
                dari <span className="font-medium">{payments.length + (currentPage - 1) * itemsPerPage}</span> hasil
              </p>
            </div>
            <div>
